...
function render(list) {
	...
	list.forEach((product) => {
		...
		discountedPrice.className = "box-discounted-price";
		image.src =
		...
		discountedPrice.textContent = `$ ${Number(product.discountedPrice).toLocaleString("en-US", { minimumFractionDigits: 2 })}`;
		link.href = `index.html?id=${product.id}`;

		function showDiscountedPrice() {
			if (product.discountedPrice < product.price) {
				price.style.textDecoration = "line-through";
				container.append(title, discountedPrice);
			} else {
